...
const TASKS_KEY = "pcs_tasks_v2";
const RENTALS_KEY = "pcs_rentals_v2";
const BUDGET_KEY = "pcs_budget_v2";
const CONTACTS_KEY = "pcs_contacts_v2";
const SALE_KEY = "pcs_house_sale_v2";
...
export default function PCSPlannerPage() {
  ...
  useEffect(() => { window.localStorage.setItem(TASKS_KEY, JSON.stringify(tasks)); }, [tasks]);
  useEffect(() => { window.localStorage.setItem(RENTALS_KEY, JSON.stringify(rentals)); }, [rentals]);
  useEffect(() => { window.localStorage.setItem(BUDGET_KEY, JSON.stringify(budget)); }, [budget]);
  useEffect(() => { window.localStorage.setItem(CONTACTS_KEY, JSON.stringify(contacts)); }, [contacts]);
  useEffect(() => { window.localStorage.setItem(SALE_KEY, JSON.stringify(milestones)); }, [milestones]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    const list = enrichedTasks.filter((t) => {
      ...
      return `${t.task} ${t.owner} ${t.notes} ${t.phase}`.toLowerCase().includes(term);
    });

    if (taskFilters.sort === "priority") return [...list].sort((a, b) => (priorityRank[a.priority] ?? 9) - (priorityRank[b.priority] ?? 9));
  ...
  const boardColumns = useMemo(() => {
    return statusOptions.map((status) => ({ status, items: filteredTasks.filter((task) => task.status === status) }));
  }, [filteredTasks]);

  const ownerLoad = useMemo(() => {
    const grouped = {};
    enrichedTasks.forEach((task) => {
      grouped[task.owner] = grouped[task.owner] || { owner: task.owner, total: 0, open: 0 };
      grouped[task.owner].total += 1;
      if (task.status !== "Complete") grouped[task.owner].open += 1;
    });
  ...
  const addTask = async () => {
    ...
    setToast("Task added.");
    if (supabase) {
      const { error } = await supabase.from("tasks").upsert({ ...created, target: created.dueDate });
      if (error) setWarning(`Supabase save warning: ${error.message}`);
    }
  };

  const quickComplete = () => {
    const candidate = enrichedTasks.find((task) => task.status !== "Complete" && (task.overdue || task.priority === "Critical"));
    if (!candidate) return;
    updateTask(candidate.id, { status: "Complete" });
  };
  ...
  return (
    <main className="app-shell">
      <section className="header">
        ...
        <div className="nav-tabs">{tabs.map((tab) => <button key={tab} className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{tab}</button>)}</div>
      </section>

      {!isSupabaseConfigured && <p className="alert">Cloud sync is off. Working locally in your browser only.</p>}
      ...
      {activeTab === "Dashboard" && (
        <section className="grid" style={{ gap: 14 }}>
          ...
          <div className="grid three">
            <div className="card">
              <h3 className="section-title">Monday Kickoff</h3>
              <p className="muted">Start with one critical win and one overdue cleanup.</p>
              <div className="controls">
                <button className="primary" onClick={() => setActiveTab("Master Plan")}>Open Task Board</button>
                <button className="ghost" onClick={quickComplete}>Mark Top Item Complete</button>
              </div>
            </div>
            <div className="card">
              <h3 className="section-title">Overdue Alerts</h3>
              {metrics.overdue ? <p className="alert">{metrics.overdue} overdue task(s) need attention.</p> : <p className="success">No overdue items right now.</p>}
            </div>
            <div className="card">
              <h3 className="section-title">Next Housing Move</h3>
              <p className="muted">Milestone progress: {milestoneProgress}%</p>
              <div className="progress"><span style={{ width: `${milestoneProgress}%` }} /></div>
              <p className="muted" style={{ marginTop: 8 }}>Top rental fit: {rentalScored[0]?.property || "N/A"}</p>
            </div>
          </div>
          ...
          <div className="grid two">
            <div className="card">
              ...
              <ul>
                {enrichedTasks.filter((t) => t.priority === "Critical" || t.overdue).slice(0, 6).map((t) => (
                  <li key={t.id} style={{ marginBottom: 8 }}>
                    <strong>{t.task}</strong> <span className={priorityClass(t.priority)}>{t.priority}</span> <span className={statusClass(t.status, t.overdue)}>{t.status}</span>
                    <div className="muted">{t.owner} • due {t.dueDate || "TBD"} • {t.phase}</div>
                  </li>
                ))}
              </ul>
            </div>
            <div className="card">
              <h3 className="section-title">Owner Workload</h3>
              {ownerLoad.map((row) => (
                <div key={row.owner} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}><strong>{row.owner}</strong><span className="muted">{row.open} open / {row.total} total</span></div>
                  <div className="progress"><span style={{ width: `${row.total ? Math.round((row.open / row.total) * 100) : 0}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === "Master Plan" && (
        <section className="card">
          <h3>Master Plan</h3>
          <p className="muted">Use table view for detail and board view for Monday-style execution planning.</p>

          <div className="controls">
            <input placeholder="Search tasks" value={taskFilters.search} onChange={(e) => setTaskFilters((f) => ({ ...f, search: e.target.value }))} />
            <select value={taskFilters.status} onChange={(e) => setTaskFilters((f) => ({ ...f, status: e.target.value }))}><option>All</option>{statusOptions.map((s) => <option key={s}>{s}</option>)}</select>
            <select value={taskFilters.owner} onChange={(e) => setTaskFilters((f) => ({ ...f, owner: e.target.value }))}><option>All</option>{taskFilterOptions.owners.map((v) => <option key={v}>{v}</option>)}</select>
            <select value={taskFilters.category} onChange={(e) => setTaskFilters((f) => ({ ...f, category: e.target.value }))}><option>All</option>{taskFilterOptions.categories.map((v) => <option key={v}>{v}</option>)}</select>
            <select value={taskFilters.phase} onChange={(e) => setTaskFilters((f) => ({ ...f, phase: e.target.value }))}><option>All</option>{taskFilterOptions.phases.map((v) => <option key={v}>{v}</option>)}</select>
            ...
            <button className="ghost" onClick={() => setTaskView((v) => (v === "Table" ? "Board" : "Table"))}>{taskView === "Table" ? "Switch to Board" : "Switch to Table"}</button>
          </div>
                ...
                <tbody>
                  {filteredTasks.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>{t.owner}</td>
                      <td><select value={t.status} onChange={(e) => updateTask(t.id, { status: e.target.value })}>{statusOptions.map((s) => <option key={s}>{s}</option>)}</select></td>
            ...
            <div className="board-grid">
              {boardColumns.map((col) => (
                <div key={col.status} className="board-column">
                  <div className="board-title">{col.status} <span className="muted">({col.items.length})</span></div>
                  {col.items.length === 0 && <p className="muted">No tasks in this lane.</p>}
                  {col.items.map((task) => (
                    <article key={task.id} className="board-card">
                      <strong>{task.task}</strong>
                      <div style={{ marginTop: 6 }}><span className={priorityClass(task.priority)}>{task.priority}</span> {task.overdue && <span className="badge status-overdue">Overdue</span>}</div>
                      <div className="muted" style={{ marginTop: 6 }}>{task.owner} • {task.phase} • due {task.dueDate || "TBD"}</div>
                      <select value={task.status} onChange={(e) => updateTask(task.id, { status: e.target.value })} style={{ marginTop: 8 }}>
          ...
          <div className="grid three" style={{ marginTop: 10 }}>
            {rentalScored.map((r) => (
              <article key={r.id} className="card" style={{ padding: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong>{r.property || "Untitled property"}</strong><span className="badge medium">Fit {r.fitScore}</span></div>
      ...
      {activeTab === "Budget" && (
        <section className="card">
          ...
          <div className="table-wrap" style={{ marginTop: 10 }}><table><thead><tr><th>Category</th><th>Estimate</th><th>Actual</th><th>Variance</th><th>Reimbursable</th><th>Notes</th></tr></thead><tbody>
            {budget.map((item) => {
              ...
              return (
                <tr key={item.id}>
                  ...
                  <td style={{ color: variance > 0 ? "#7b2d3a" : "#5a434a" }}>${variance.toLocaleString()}</td>
          ...
          <div className="grid three" style={{ marginTop: 10 }}>
            ...
            <div className="card"><div className="muted">Total Variance</div><div className="kpi-value" style={{ color: budgetSummary.variance > 0 ? "#7b2d3a" : "#5a434a" }}>${budgetSummary.variance.toLocaleString()}</div></div>
          </div>
        </section>
      )}
      ...
      {activeTab === "Contacts" && (
        <section className="card">
          ...
          <div className="table-wrap" style={{ marginTop: 10 }}><table><thead><tr><th>Role</th><th>Name</th><th>Phone</th><th>Email</th><th>Location</th><th>Status</th><th>Notes</th></tr></thead><tbody>
            {contacts.map((c) => (
              <tr key={c.id}>
      ...
      {activeTab === "Onboarding" && (
        <section className="grid two">
          <article className="card">
            ...
            <p className="muted">Think of this page like your family’s Monday brief: check the dashboard, then move cards forward in Master Plan.</p>
            <ul>
              <li>Dashboard = health snapshot + Monday kickoff actions</li>
              <li>Master Plan = board/table execution engine</li>
              <li>House Sale / Rental = housing decision workflow</li>
              <li>Budget / Contacts = support systems</li>
            </ul>
          </article>
          <article className="card">
            <h3>How progress is calculated</h3>
            <p className="muted">Progress = completed tasks / total tasks. Overdue is based on due date and non-complete status. Rental fit uses budget + commute + bedrooms + shortlist bonus.</p>
            <div className="grid">
              <div><span className="badge status-complete">Complete</span> contributes to completion rate.</div>
              <div><span className="badge status-overdue">Overdue</span> signals urgent cleanup.</div>
              <div><span className="badge critical">Critical</span> is highest urgency.</div>
            </div>
          </article>
        </section>
      )}
