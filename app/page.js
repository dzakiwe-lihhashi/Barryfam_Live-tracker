...
import { hasSupabaseAnonKey, hasSupabaseUrl, supabase, isSupabaseConfigured } from "../lib/supabaseClient";
...
const tasksTableSql = `create table if not exists public.tasks (
...
export default function PCSPlannerPage() {
  ...
  return (
    <main className="app-shell">
      ...
      {!isSupabaseConfigured && (<CloudSyncSetupCard hasSupabaseUrl={hasSupabaseUrl} hasSupabaseAnonKey={hasSupabaseAnonKey} />)}
...
function CloudSyncSetupCard({ hasSupabaseUrl, hasSupabaseAnonKey }) {
  const missing = [];
  ...
  const copySql = async () => {
    try {
      await navigator.clipboard.writeText(tasksTableSql);
    } catch {
  ...
  return (
    <div className="card" style={{ marginBottom: 10 }}>
      <h3 style={{ marginTop: 0 }}>Cloud sync is off (local mode)</h3>
      <p className="muted" style={{ marginTop: 0 }}>
        Your planner is currently saving only to this browser. To enable shared cloud sync, add the missing env vars and restart the app.
      </p>
      ...
      <ol style={{ marginTop: 8, paddingLeft: 18 }}>
        ...
        <li>Set both env vars listed above with your Supabase project values.</li>
        <li>Run the SQL setup for <code>public.tasks</code> in Supabase SQL Editor.</li>
        <li>Restart <code>npm run dev</code> and refresh this page.</li>
      </ol>
      <div className="controls" style={{ marginTop: 8 }}>
        <button className="ghost" onClick={copySql}>Copy tasks table SQL</button>
      </div>
    </div>
  );
}
